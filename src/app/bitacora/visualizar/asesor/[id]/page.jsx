...
function page({ params }) {
    ...
    const traerEstadoSeguimiento = async (id) => {
        const response = await fetch('http://localhost:3002/estado-seguimiento-cambio/Seguimiento/' + id);
        const data = await response.json();
        return data
    }
    return (
                                        ...
                                        return (
                                            <tr key={item.id}>
                                                ...
                                                <td className="whitespace-nowrap px-4 py-2 font-semibold text-gray-400 text-center">{
                                                    ...
                                                    {estados[0].estadoSeguimiento.id == 1 && new Date(estados[0].fecha).getDate() == new Date("04/26/2024").getDate() ?
                                                        (
                                                            <a href={'/seguimientos/modificar/' + item.id + '/' + estados[0].id} className=' flex items-center justify-center'>
